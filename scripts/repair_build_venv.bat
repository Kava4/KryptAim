...
if not exist "%PY%" (
    echo Run scripts\create_build_venv.bat first.
    pause
    exit /b 1
)

echo ============================================================
echo  Repair build-venv AI deps (CUDA torch for PyInstaller)
echo ============================================================
echo Close AimSync / PyInstaller if running, then press a key...
pause >nul
