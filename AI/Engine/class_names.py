...
logger = logging.getLogger('AimSync.AI.ClassNames')

# Filename heuristics when Ultralytics/metadata unavailable (cs2_640.onnx → CT, T).
...
def _names_from_ultralytics(model_path: Path) -> dict[int, str] | None:
    ...
    try:
        ...
        if names:
            return _normalize_names(names, len(names) if isinstance(names, dict) else len(names))
        ...
        for prop in model.metadata_props:
            ...
            if isinstance(raw, str):
                try:
                    ...
                    return _normalize_names(parsed, len(parsed) if isinstance(parsed, list) else len(parsed))
    except Exception as exc:
...
def resolve_class_names(model_path: Path, num_classes: int = 1) -> dict[int, str]:
    """Best-effort class labels: Ultralytics metadata → ONNX metadata → filename heuristics."""
